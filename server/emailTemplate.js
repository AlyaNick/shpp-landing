...
/**
 * @param {{ name: string, email: string, sphere: string, request: string }} data
 */
export function buildRegistrationEmailHtml(data) {
  const name = escapeHtml(data.name || '—')
  const email = escapeHtml(data.email || '—')
  const sphere = escapeHtml(data.sphere || '—')
  const request = escapeHtml(data.request || '—').replace(/\n/g, '<br>')
  const date = escapeHtml(formatDate())
